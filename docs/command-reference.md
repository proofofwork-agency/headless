...
Default help contains only the Beta 1 execution and project-lifecycle boundary. Every other command requires the explicit `headless experimental` namespace and has no stability promise.
...
| `init` | Beta 1 | — | `init [--cwd dir]                 Initialize external per-project state without modifying the checkout.` |
| `status` | Beta 1 | — | `status [--cwd dir]              Show project and daemon status.` |
| `doctor` | Beta 1 | — | `doctor [--cwd dir]              Check backend, containment, and daemon readiness.` |
| `mcp` | Experimental | — | `experimental mcp <serve|install|remove|status> [codex|grok|claude|opencode]` |
