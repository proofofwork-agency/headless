...
| CI | Current macOS and Linux jobs exercise real containment and the complete release check, but not every containment case runs on both legs, and a green workflow is not evidence that it did. Two exceptions are declared rather than waived: the four daemon run-tool cooperation/delegation cases are skipped on GitHub-hosted Linux (covered on macOS CI, local dev, and the privileged Docker command in [hosted-linux-relay-follow-up.md](./internal/hosted-linux-relay-follow-up.md)), and the late-created-host-socket relay-lifecycle case runs on **no** CI leg — macOS skips it as Linux-only and hosted Linux skips it for the same relay incompatibility. `tests/gated-coverage.test.ts` is the authoritative registry of which leg runs each capability gate and fails when a gate is uncovered without a stated reason. macOS CI explicitly enables the sanitized public DNS/TLS Seatbelt probe; ordinary local tests retain deterministic local IP/Unix-socket coverage without public network. The installed-CLI native-subscription smoke is the primary opt-in real-run gate; the provider-key broker smoke remains a separate, optional/deferred protected job (not required for the native-login beta). |
