...
The complete current branch, including depth-one contained delegation, has passed the hosted macOS and Linux workflow. That evidence closes the former CI-bootstrap blocker but is not a substitute for the native-subscription smoke above, and it must be re-run after relevant changes.
