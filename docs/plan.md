...
1. **Native-subscription smoke — primary Gate A real-run evidence.** `HEADLESS_NATIVE_SMOKE=1 bun run smoke:native` must pass without API keys for the installed, already-logged-in required backends — Claude, Codex, and OpenCode. Each run omits model, remains read-only and required-contained, reports native-direct-unrestricted/backend-native evidence, leaves the primary checkout unchanged, and retains the selected driver/version/auth fingerprint without auth contents. The gate is **per-backend**: Grok is experimental and excluded; macOS keychain-only Claude login is a documented accepted limitation — a platform-gated skip that does not fail the gate (Claude runs natively on Linux/file-credential logins). At least one required backend must complete a real native turn; a missing required backend or a changed checkout fails the gate. See [native-login.md](./native-login.md#opt-in-real-subscription-smoke). (Gate A)
