...
1. **Native-subscription smoke — primary Gate A real-run evidence.** `HEADLESS_NATIVE_SMOKE=1 bun run smoke:native` must pass without API keys for the installed, already-logged-in required backends. Each run omits model, remains read-only and required-contained, reports native-direct-unrestricted/backend-native evidence, leaves the primary checkout unchanged, and retains the selected driver/version/auth fingerprint without auth contents. The gate is **per-backend and platform-aware**: on macOS the required set is Codex and OpenCode (macOS Claude Code keeps its live token in the login Keychain, which required containment cannot read, so Claude is a documented limitation there); on Linux a file-credential Claude joins Codex and OpenCode. Grok is experimental and never required. At least one required backend must complete a real native turn. A required backend that is only transiently rate-limited (authenticated and contained, but subscription-throttled this run) does not fail the gate and is flagged for a strict re-run; a genuine failure, a missing required backend, or a changed checkout fails the gate. See [native-login.md](./native-login.md#opt-in-real-subscription-smoke). (Gate A)
2. The recorded lead→deliberate→council end-to-end smoke described under Gate B. (Gate B)
