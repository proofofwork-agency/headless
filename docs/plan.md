...
- Worker `run.delegate` admission is daemon-derived and non-queueing: one parent-deadline-bounded, depth-one read-only child, no native credentials or delegate operation on the child, and no new spend authority. Same-provider children use an atomic parent sub-reservation; cross-provider children require different providers and backends plus a strict `broker-api-key` target and use a crash-atomic linked hold over the parent and target provider quotas.
...
## Gate A — kernel beta (`0.2.0-beta.3`)
...
| Ledger/state | Reserved-field protection, incremental verified reads, auditor-runnable full-chain/evidence verification, partial lines, exact full-prefix duplicate suppression, ownership-aware locks, digest-bound ledger and semantic context/task projections, provenance-anchored release evidence, and valid/invalid/crash-restarted v1 migration. |
...
| CI | Current macOS and Linux jobs exercise real containment and the complete release check, but not every containment case runs on both legs, and a green workflow is not evidence that it did. Both former exceptions are closed as of 2026-08-05: the four daemon run-tool cooperation/delegation cases and the late-created-host-socket relay-lifecycle case now run on the ordinary hosted Linux leg, having previously been skipped there for what was believed to be a hosted-runner incompatibility and was in fact our own relay discarding early requests (see [hosted-linux-relay-follow-up.md](./internal/hosted-linux-relay-follow-up.md)). `tests/gated-coverage.test.ts` is the authoritative registry of which leg runs each capability gate and fails when a gate is uncovered without a stated reason. macOS CI explicitly enables the sanitized public DNS/TLS Seatbelt probe; ordinary local tests retain deterministic local IP/Unix-socket coverage without public network. The installed-CLI native-subscription smoke is the primary opt-in real-run gate; the provider-key broker smoke remains a separate, optional/deferred protected job (not required for the native-login beta). |
...
| Collaboration | Iterative questions/revisions, addressed acknowledged messages, actual artifact/diff delivery, sticky task synthesis and health failover without authority promotion, active-lead backend exclusion for automatic routing, optional election, four-worker/64-queue defaults, overflow, deterministic idle opportunities, rate-limit requeue bounds, approval modes, finality enforcement, and bounded worker delegation with atomic parent-budget inheritance through same-provider sub-reservations or cross-provider linked holds. |
...
Headless is exercised through the CLIs' existing subscription logins (native-login). The kernel beta's real-run evidence is the native-subscription smoke below; **no gate in this beta requires separate provider API keys.** These must be closed or the affected gate must remain unpublished:

1. **Native-subscription smoke — primary Gate A real-run evidence (recorded).** `HEADLESS_NATIVE_SMOKE=1 bun run smoke:native` must pass without API keys for the installed, already-logged-in required backends. Each run omits model, remains read-only and required-contained, reports native-direct-unrestricted/backend-native evidence, leaves the primary checkout unchanged, and retains the selected driver/version/auth fingerprint without auth contents. The gate is **per-backend and platform-aware**: on macOS the required set is Codex and OpenCode; Claude may additionally use the explicit `~/.claude/.headless-setup-token` capsule because required containment cannot import a login-Keychain token. On Linux a file-credential Claude joins Codex and OpenCode. Grok is experimental and never required. At least one required backend must complete a real native turn. A required backend that is only transiently rate-limited (authenticated and contained, but subscription-throttled this run) does not fail the gate and is flagged for a strict re-run; a genuine failure, a missing required backend, or a changed checkout fails the gate. The recorded artifact reports `releaseGatePassed: true`, a real OpenCode pass, transient Claude/Codex rate limits after repeated attempts, the documented Grok attestation skip, and an unchanged repository; the independent write smoke records genuine contained Claude/Codex/OpenCode turns. See [native-login.md](./native-login.md#opt-in-real-subscription-smoke), [native-subscription-smoke.json](./internal/release-evidence/native-subscription-smoke.json), and [native-write-smoke.json](./internal/release-evidence/native-write-smoke.json). (Gate A/C evidence; publication remains gated by the full final tree checks.)
2. **Lead→deliberate→council end-to-end smoke (recorded).** A real Codex lead attached through MCP stdio, fanned out native-login deliberation to Claude Code, OpenCode, and Grok, then ran a Claude Code/OpenCode council through all four phases. The council produced an attributable negative boolean decision with two terminal, parseable votes; the artifact, exact council record, and all 11 job completions matched both the ledger and observer events projection after an authenticated daemon stop/restart. The decision polarity is evidence, not a publication verdict. See [gate-b-mcp-smoke.json](./internal/release-evidence/gate-b-mcp-smoke.json). (Gate B evidence; Gate B remains unpublished until its complete final-tree release check passes.)

**Optional / deferred — broker mode with provider API keys.** The manually triggered protected provider-smoke workflow exercises one bounded real broker request per provider (OpenAI, Anthropic, Gemini, xAI) through a scoped lease; its `provider-smoke` GitHub environment holds secrets `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `XAI_API_KEY` and variables `HEADLESS_SMOKE_OPENAI_MODEL`, `HEADLESS_SMOKE_ANTHROPIC_MODEL`, `HEADLESS_SMOKE_GEMINI_MODEL`, `HEADLESS_SMOKE_XAI_MODEL`. It proves the broker protocol against real upstreams but is **not required for the native-login subscription beta** — the broker is already proven by the fake-upstream unit tests. Close it if and when broker mode is advertised for API-key users; the repository makes no claim the credentialed run has passed.

The complete current branch has passed the hosted macOS and Linux workflow, and as of 2026-08-05 **depth-one contained delegation is exercised on hosted x86-64 Linux as well as macOS**. The four daemon run-tool cooperation cases and the late-created-host-socket relay-lifecycle case now run unguarded on the ordinary Ubuntu release gate; `tests/gated-coverage.test.ts` declares them on both legs and no longer records either exclusion.

This paragraph previously stated the opposite, and named those exclusions as a condition that **must be closed or covered on real Linux before Gate B publishes delegation**. That condition is now met, and it is worth recording why it stood so long: the exclusions were attributed to a GitHub hosted-runner incompatibility, and that attribution was wrong. The cause was our own Linux relay discarding any run-tool request that arrived before its upstream Unix connect resolved — a race that fast hosts win, which is why only the hosted x86-64 leg ever failed. Until it was fixed, those five cases had no passing x86-64 Linux evidence: they were excluded from ordinary CI, and the instrumented diagnostic runs that did force them there failed. See [hosted-linux-relay-follow-up.md](./internal/hosted-linux-relay-follow-up.md) for the instrumented evidence and the corrections.

That evidence closes the former CI-bootstrap blocker but is not a substitute for the native-subscription smoke above, and it must be re-run after relevant changes.

Do not carry forward an older local release pass after control-plane, native-auth, session-driver, fleet, TUI, or package changes. Re-run `release:check`, clean tarball installation, compiled `headless`/`hless`/`headless-mcp`/daemon/TUI/plugin startup, real Seatbelt, privileged bubblewrap/seccomp, and generated-artifact cleanliness from the final tree. The repository makes no claim that the native-subscription smoke has passed until its evidence is recorded.
