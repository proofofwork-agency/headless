...
The complete current branch has passed the hosted macOS and Linux workflow. Read that pass precisely: depth-one contained delegation is exercised by the hosted **macOS** leg only, because its four daemon run-tool cooperation cases are skipped on GitHub-hosted Linux, and the late-created-host-socket relay-lifecycle case runs on neither hosted leg. Both exclusions are recorded in `tests/gated-coverage.test.ts` and tracked in [hosted-linux-relay-follow-up.md](./internal/hosted-linux-relay-follow-up.md); they are CI incompatibilities, not containment waivers, and they must be closed or covered on real Linux before Gate B publishes delegation. That evidence closes the former CI-bootstrap blocker but is not a substitute for the native-subscription smoke above, and it must be re-run after relevant changes.
