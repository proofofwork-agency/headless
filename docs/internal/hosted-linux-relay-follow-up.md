...
Status: RESOLVED (2026-08-04) — both the four cooperation cases and the combined late-socket case. One root cause, in our own relay. This is a private-beta CI/runtime investigation, not a containment waiver.
...
## Also resolved: the combined late-socket case

Same cause, measured separately rather than assumed. Its failures always read `brokerStatus` 200 with `toolCode` 1 — the broker leg healthy, only the run-tool leg broken. The broker uses the HTTP proxy, which pipes synchronously and never had the discard window; only the run-tool leg traversed the defective stream proxy.

Its `unixError` ENOENT was never part of that fault: that is the deliberately late host socket refusing to connect, resolved before the helper is ever spawned, so the bridge cannot produce it. Reading those two fields as one signature is what made this look like a separate problem.

Because the old fault was a race, one green run would prove nothing. Sampled 10× on hosted x86-64 under the override: **10 pass, 0 fail, 0 absent** (run 30956521650), against 3 of 9 failing before the fix. The gate is removed and the case now runs on the ordinary ubuntu leg and in the privileged workflow.

The privileged workflow previously asserted this case appears as `(skip)`, as a witness that `GITHUB_ACTIONS` reached the container. That witness is gone with the predicate, so it now asserts `(pass)` — a strictly stronger statement about the same case.
...
- The **combined** case (`… while broker and run tools remain reachable`) now RUNS on hosted Linux; it was skipped there until 2026-08-04 because its run-tool leg was intermittent, which was the relay defect above. Historical raw evidence, retained because it is what identified the leg: 3 of 9 hosted samples failed, every diagnosed one with `unixError` ENOENT, `brokerStatus` 200 and `toolCode` 1 — containment held, only run-tool availability broke (runs 30941830730, 30942404411, 30942472935). After the relay fix: 10 of 10 pass (run 30956521650).
...
- `HEADLESS_PRIVILEGED_CONTAINMENT_CI` no longer gates any test. It was the opt-in that let a privileged or self-hosted runner execute the cases the hosted predicate excluded, and both of those predicates are gone. The privileged workflow now asserts the combined case appears as `(pass)`.

All five cases now run in ordinary CI, so nothing below is load-bearing coverage any more. The command is kept because it is the only way to exercise them against real bubblewrap on a maintainer's own machine, and because its anti-vacuity assertions are the reason the earlier gaps were visible at all. It asserts all five cases by name:
...
**Why every case is asserted by name instead of trusting the exit code.** A `bun test` run whose cases were all gated out prints `0 pass 4 skip 0 fail` and exits 0, which is indistinguishable from success at a glance — that vacuous pass is what this file used to hand a maintainer. Emulation produces it too: without a correct `--platform`, Docker on Apple Silicon reuses a cached `linux/amd64` image, emulated bubblewrap fails `strictContainmentAvailable()`, and every gated case skips. Hardcoding `linux/arm64` would move the same failure onto x86-64 hosts, so the platform is derived from `uname -m`. Asserting only the late-socket case was not enough either: at the time, re-running the suite with `GITHUB_ACTIONS=true HEADLESS_PRIVILEGED_CONTAINMENT_CI=1` skipped exactly the four cooperation cases while the late-socket case still passed, and the single-case version of this command printed `OK` and exited 0 on that log. That override is inert now — nothing gates on it — but the lesson is why all five cases are asserted by name. "Read the output carefully" is not fail-loud; the probe and the five anchored assertions are.

**Off-CI evidence as of `f78a78d` (HISTORICAL — the relay and two test files have changed since; re-run to refresh)** — this command run VERBATIM (extracted from the fenced block programmatically, not retyped), on Docker 27.4.0, `--platform linux/arm64`, real `bubblewrap 0.11.0`, privileged:
...
**Hosted x86-64 status, HISTORICAL (pre-2026-08-05).** This records the shape while the combined case was still skipped; the workflow now requires it to PASS. `.github/workflows/privileged-containment.yml` ran `tests/containment-v2.test.ts` in a privileged `oven/bun:1.3.14` container on `ubuntu-latest` with `CI=true GITHUB_ACTIONS=true` and, deliberately, WITHOUT `HEADLESS_PRIVILEGED_CONTAINMENT_CI`, which armed the hosted predicate while leaving the combined case skipped. A representative run of that era:
...
Both are now closed and measured. Do not weaken bubblewrap, seccomp, socket masking, depth-one admission, or bounded runtime deadlines to satisfy the runner.
