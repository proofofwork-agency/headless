...
- **Depth-one contained delegation is now verified on x86-64 Linux in CI.** The four `daemon-run-tool` cooperation cases and the combined `containment-v2` late-socket case were skipped on all GitHub Linux, attributed to a hosted-runner incompatibility. That attribution was wrong — the cause was the relay defect above — and until it was fixed those five cases had never executed on x86-64 Linux at all. They now run unguarded on the ordinary Ubuntu release gate. The combined case was sampled 10 times on hosted x86-64 before its guard was removed, against 3 of 9 failing beforehand.
