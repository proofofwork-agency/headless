...
- **The `HEADLESS_` worker-environment boundary is deny-by-default.** Only an explicit allowlist of run-scoped capabilities and non-secret run identity reaches a worker; every other `HEADLESS_` variable is dropped, including ones invented after this change. It was previously allow-by-default with a denylist, and that shape failed three times: `HEADLESS_LEDGER_KEY` was denied by name, then the keyring variables (`HEADLESS_LEDGER_KEYS`, `_KEY_ID`, `_ACTIVE_KEY_ID`) were added and silently defaulted back to allowed, and the extension family — which selects the daemon extension modules to **execute** and carries their pinned path and hash — was still denied by name only. Inverting the default makes every variable written from now on daemon-only on the day it is written. Verified: a newly invented `HEADLESS_SOMETHING_SECRET` is dropped with nobody adding it to a list, and the ledger keyring, extension config and `HEADLESS_STATE_HOME` are all dropped. The namespace decision is made **before** the caller's `credentialPrefixes` arm, so even a caller passing a bare `HEADLESS_` prefix cannot re-admit daemon-only material. **If you rely on an ambient `HEADLESS_` variable surviving `buildAdapterEnv` into a worker, it must now be listed in `WORKER_HEADLESS_KEYS` in `src/backends/env.ts`.** This governs variables inherited from the source environment; per-run capabilities injected *after* this boundary — `HEADLESS_BROKER_TOKEN` and `HEADLESS_BROKER_UNIX_SOCKET` (`src/runner/simple.ts`) — still reach the worker and are unaffected.
...
- **The Linux relay no longer discards a run-tool request that arrives before its upstream connects.** `startStreamProxy` paused the accepted client and attached the pipe only from the upstream `connect` callback, so bytes landing in that window had no consumer and were dropped. Nothing errored on any side: the worker helper reported a connected transport, the relay reported an accepted client and a connected upstream, and the daemon held an accepted connection that never received a frame — so the worker waited out its full deadline and reported `run tool unavailable: run tool timeout`. It is a race, not a platform property: whichever side wins decides whether the window is entered, and raising the run-tool timeout from 15 to 30 to 60 seconds never converged because the request was already discarded. Observed only on the hosted x86-64 leg; arm64 passed even starved to 0.4 CPUs, so general load alone does not reproduce it and no broader claim about "loaded runners" is made. The request was already gone. The consumer is now attached synchronously at accept and buffers, bounded, until the upstream is connected.
- **Depth-one contained delegation is now verified on x86-64 Linux in CI.** The four `daemon-run-tool` cooperation cases and the combined `containment-v2` late-socket case were skipped on all GitHub Linux, attributed to a hosted-runner incompatibility. That attribution was wrong — the cause was the relay defect above. Until it was fixed those five cases had no passing x86-64 Linux evidence: they were excluded from ordinary CI, and the instrumented runs that forced them to execute there failed. They now run unguarded on the ordinary Ubuntu release gate. The combined case was sampled 10 times on hosted x86-64 before its guard was removed, against 3 of 9 failing beforehand.
- **A run-tool call is no longer cut off by the window that bounds its own request.** The daemon set a 5s inactivity timeout when it accepted the connection, and `socket.setTimeout` is an *inactivity* timer — a handler that is running produces no socket traffic. Any call whose work outlasted 5s had its healthy connection destroyed with `run tool connection closed without a response`. The window is now re-based on frame completion to the credential's remaining lifetime, still bounded and still reclaimed by revocation.
