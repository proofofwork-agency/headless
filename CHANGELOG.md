...
- Moved the hosted-Ubuntu run-tool cooperation lifecycle coverage into a privileged Linux container step. GitHub's unprivileged userns/AppArmor runner can leave bwrap relay children alive after a completed tool response, so the ordinary hosted process loudly skips exactly four daemon-cooperation cases and one late-socket relay case while macOS, local Linux, and the privileged Linux CI step continue to execute them. The underlying hosted-runner relay hang and unrelated `Unknown goal` background-error leakage remain explicitly tracked in `docs/internal/hosted-linux-relay-follow-up.md`.
