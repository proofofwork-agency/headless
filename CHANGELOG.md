...
- Native-login authentication is the default after explicit project trust; brokered API-key mode must be selected explicitly. Approval policy is a separate `ask`, `auto`, or contained `bypass` choice.
...
- Provider-direct containment for native-login workers while preserving the existing outer Seatbelt or bubblewrap/seccomp filesystem, process, Unix-socket, worktree, and repository boundaries. Native results report backend-native credential access and unknown cost unless the CLI supplies a charge; broker mode remains broker-only and cost-enforced.
...
- Codex one-shot, exec-resume, and app-server paths share a fail-closed policy that disables project plugins/hooks, native multi-agent fan-out, apps/browser/computer-use surfaces, MCP skill dependencies, and bounded `.agents/skills` plus `.codex/skills` entries. Required Grok 0.2.93 launches remain explicitly blocked because its late project-control reload surface cannot yet be lifetime-contained on both release platforms.
